...
module.exports.controller = function (app) {
	...
	if (req.user) {
	    res.json({ username: req.user.username, verified: req.user.verified });
	} else {
	    res.json({});
	}
    });
  ...
  app.get('/email', function (req, res) {
    if (req.query.url) {
        req.session.attemptedURL = req.query.url;
    }
  ...
  app.post('/email', function (req, res, next) {
    ...
    workflow.on('verification', function () {
      ...
      crypto.randomBytes(25, function (err, buf) {
        ...
        // next step
        workflow.emit('updateProfile', verifyToken);
      });

    });

    /****************************************************************/    
    workflow.on('updateProfile', function (verifyToken) {
      User.findById(req.user.id, function (err, user) {
        if (err) {
          return next(err);
        }
	
        req.user.email = user.email = req.body.email.toLowerCase() || '';
        req.user.verifyToken = user.verifyToken = verifyToken;
        ...
        user.save(function (err) {
          ...
          // next step, pass user
	  workflow.emit('sendValidateEmail', verifyToken);
        });
      });
    });
    ...
    workflow.on('sendValidateEmail', function (verifyToken) {
  ...
  app.post('/login', function (req, res, next) {
    ...
    workflow.on('validate', function () {
      ...
      req.assert('username', 'Your code cannot be empty.').notEmpty();
    ...
    workflow.on('abuseFilter', function () {
      ...
      var getIpUserCount = function (done) {
        var conditions = { ip: req.ip, user: req.body.username.toLowerCase() };
        LoginAttempt.count(conditions, function (err, count) {
          if (err) {
            return done(err);
          }
    ...
    workflow.on('authenticate', function () {
      ...
      passport.authenticate('local', function (err, user, info) {
        ...
        if (!user) {
          ...
          // Update abuse count
          var fieldsToSet = { ip: req.ip, user: req.body.code };
